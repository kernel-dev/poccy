  ...
  )
{
  ...
  for (
    UINT32 Index = 0; Index < (Xsdt->Length - 0x2C) / sizeof (UINT64); ++Index
    )
  {
    ...
    if (KernMemCmp (sigStr, "APIC", 4) == 0) {
      kprintf (
        "[DEBUG::INTERRUPTS::MADT]: Found MADT (address: %p, offset to entries: %p) -> Signature: %s\n\n",
        Table,
        (Table + 0x2A),
        sigStr
        );
  ...
  kprintf (
    "[DEBUG::INTERRUPTS::MADT]: MadtData:\n\tNumber of cores: %x | IO APIC Pointer: %p | Local APIC Pointer: %p\n",
    MadtData->NumberOfCores,
    MadtData->IOAPICPointer,
